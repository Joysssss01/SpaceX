...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        fig = px.pie(filtered_df, names='class', title='Success Count for all launch sites')
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        filtered_df = filtered_df.groupby('class').size().reset_index(name='count')
        fig = px.pie(filtered_df, names='class', values='count', title=f"Total Success Launches for site {entered_site}")
